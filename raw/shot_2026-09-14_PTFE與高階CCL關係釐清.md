---
tags:
  - source
  - screenshot
type: screenshot
date_captured: 2026-09-14
image: assets/instagram_2026-09-01_PTFE與高階CCL關係釐清_01.jpg
original_url: "https://www.instagram.com/p/Dcl1dc0GHxG/?igsi=MTE4Zjdtd2tnczhvYw=="
image_count: 2
expected_min: 6
captured_by: fetch_media
source_note: "instagram_2026-09-01_PTFE與高階CCL關係釐清.md"
status: 已讀圖
---

# 📸 原貼文圖片 2026-09-14 — instagram_2026-09-01_PTFE與高階CCL關係釐清

> 原始出處：https://www.instagram.com/p/Dcl1dc0GHxG/?igsi=MTE4Zjdtd2tnczhvYw==
> 對應筆記：[[instagram_2026-09-01_PTFE與高階CCL關係釐清]]

**取得方式**：`tools/fetch_media.py` 從 Jina Reader 回傳的 markdown 取出主文圖片網址並下載。
n8n 管道把這些網址丟掉了，所以由本機補。

⚠️ **這組輪播沒抓齊**：圖面自己印了 `1/6`、`2/6`，共 6 張，這裡只有 2 張。
登出狀態的 IG 只吐得出前一兩張，其餘要用已登入的 Chrome 開貼文逐張補。
（注意：這篇的網址沒有 `img_index` 參數，所以自動偵測抓不到，是靠讀圖才發現的。）

## 圖片

![[instagram_2026-09-01_PTFE與高階CCL關係釐清_01.jpg]]

![[instagram_2026-09-01_PTFE與高階CCL關係釐清_02.jpg]]

## 內容

**「PTFE 不是 CCL 終結者——一篇看懂 AI 伺服器材料換代」** — 股海書生（STOCK_SCHOLAR）。
原輪播共 **6 張**，這裡只抓到 **1/6 與 2/6**（登出狀態的 IG 只吐得出前幾張，詳見上方警告）。

### 圖 1/6｜封面
三個標籤：**無玻纖 PTFE**／**2027 量產目標**／**驗證未定案**

> 核心句：**「市場先交易替代，工程要先回答能不能量產。」**

### 圖 2/6｜先拆掉一個誤會
**CCL ＝ 銅箔 ＋ 介電樹脂 ＋ 補強材料**

分解圖標示：**PTFE ＝ 介電樹脂**（也就是 CCL 三層結構中的中間層）。

結論：**PTFE 也能做成 CCL。不是 PTFE 對決 CCL，而是不同材料組合在競爭。**

---

⚠️ **這篇沒抓齊**：後面 3/6～6/6 應該才是實質內容（推測涵蓋 PTFE 的優缺點、
與 M8／Low Dk 材料的比較、2027 量產時程與驗證進度、以及受惠／受害名單）。
需要用已登入的 Chrome 開原貼文逐張補。

**已抓到的兩張仍有價值**：它釐清了一個常見誤解——PTFE 與 CCL 不是替代關係，
PTFE 是 CCL 裡「介電樹脂」那一層的選項之一。這一點可直接寫進
[[segments/CCL]] 的名詞說明，並修正把「PTFE 取代 CCL」當成鏈斷裂風險的說法。
可與 [[JPM估PTFE僅佔CCL一成]] 對照閱讀。
