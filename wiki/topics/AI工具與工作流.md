...
### 🤖 2026-09-14 自動消化：AI 網站可讀性（內容未取得）

來源：`raw/shot_2026-09-14_AI網站評分服務.md`

⚠️ **這是 Instagram Reel（影片），只取得封面縮圖，實質內容在語音裡，未取得。**

封面可辨識的只有：標題字卡 **「AI 看不懂你的網站」**、字幕 **「你知道Nike的網站AI」**（句子未完）。

從標題**推測**主題與 AEO／GEO（AI 搜尋最佳化，即網站內容能否被 AI 模型正確讀取與引用）
或網站 AI 可讀性評分服務有關，**但這是推測，封面沒有任何可佐證的資訊**。

**不足以寫成筆記。** 若這個主題對個人網站專案有用，建議直接找原 Reel 看完再記。
