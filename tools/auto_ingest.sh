...
9. **消化完要幫來源檔改名**（n8n 存進來的檔名是亂碼 ID，看不出內容）：
   - 目標格式：<平台>_<日期>_<主題>.md，例如 threads_2026-05-27_桃機P4停車攻略.md
   - 主題 20 字以內、看得懂、不要標點符號
   - 改名前先在該檔 frontmatter 加一行 original_id: <原本的亂碼>，保留可追溯性
   - 用 git mv 改名（保留歷史），失敗就用一般搬移
   - 改完務必更新所有引用：wiki/ 各頁、index.md、data/ingested.json、
     data/known_issues.json 裡出現舊檔名的地方
   - 已經是看得懂的主題名（不含亂碼）就不要動
